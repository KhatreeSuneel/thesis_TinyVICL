...
class PairwiseConvAvgModelBlock(DefaultUnetStageBlock):
    ...
    def __post_init__(self):
        super().__post_init__()
        self.context_conv = Vmap(
            ResidualUnit(channels=self.channels, dim=self.dim, conv_layers=self.kwargs["conv_layers_per_stage"])
        )
        self.target_conv = ResidualUnit(
            channels=self.channels, dim=self.dim, conv_layers=self.kwargs["conv_layers_per_stage"]
        )
        conv_fn = getattr(nn, f"Conv{self.dim}d")
        self.combine_conv_target = Vmap(
            conv_fn(in_channels=2 * self.channels, out_channels=self.channels, kernel_size=1, padding="same")
        )
        self.combine_conv_context = Vmap(
            conv_fn(in_channels=2 * self.channels, out_channels=self.channels, kernel_size=1, padding="same")
        )

    def forward(self, context: torch.Tensor, target: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """The forward pass of an unet-stage block.
        ...
        # concat on channels
        context_target = torch.concat([context, target.unsqueeze(1).expand_as(context)], dim=2)  # B,L,2C,...
...
class PairwiseConvAvgModelOutput(DefaultUnetOutputBlock):
    ...
    def __post_init__(self):
        ...
        self.block = nn.Sequential(
            ResidualUnit(channels=self.in_channels, dim=self.dim, conv_layers=self.kwargs["conv_layers_per_stage"]),
...
if __name__ == "__main__":
    device = "cuda"
    unet2d = PairwiseConvAvgModel(
        dim=2, stages=4, in_channels=2, out_channels=3, inner_channels=32, conv_layers_per_stage=2
    ).to(device)
    ...
    unet3d = PairwiseConvAvgModel(
        dim=3, stages=4, in_channels=2, out_channels=3, inner_channels=32, conv_layers_per_stage=2
    ).to(device)
