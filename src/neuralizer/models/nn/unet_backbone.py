...
class DefaultUnetOutputBlock(nn.Module):
    ...
    def __post_init__(self):
        ...
        self.block = nn.Sequential(
            conv_fn(in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=1, padding="same")
        )
...
class UnetBackbone(nn.Module):
    ...
    def _build(self):
        conv_fn = getattr(nn, f"Conv{self.dim}d")
        self.target_embedding = conv_fn(
            in_channels=self.in_channels, out_channels=self.inner_channels[0], kernel_size=1, padding="same"
        )
        if self.context_filled:
            self.context_embedding = Vmap(
                conv_fn(
        ...
        self.output_block = self.output_block_cls(
            in_channels=self.inner_channels[0], out_channels=self.out_channels, dim=self.dim, kwargs=self.kwargs
        )

        for i in range(self.stages):
            self.enc_blocks.append(
                self.unet_block_cls(channels=self.inner_channels[i], dim=self.dim, kwargs=self.kwargs)
            )
            if i < self.stages - 1:
                self.dec_blocks.append(
                    self.unet_block_cls(channels=self.inner_channels[-(i + 2)], dim=self.dim, kwargs=self.kwargs)
                )
    ...
    def forward(
        ...
        """
        # concat context
        context = torch.cat([context_in, context_out], dim=2) if self.context_filled else context_in  # BLCHW
        # apply input embeddings
        context = self.context_embedding(context) if self.context_filled else context  # BCHW1
        ...
        for i in range(self.stages):
            ...
            # downsample
            if i < self.stages - 1:
                context, target, shortcut = self.downsample_blocks[i](context, target)
        ...
        for i in range(self.stages - 1):
            ...
            context, target = self.upsample_blocks[i](context, target, shortcuts[i])
...
class UnetDownsampleAndCreateShortcutBlock(nn.Module):
    ...
    def __post_init__(self):
        ...
        if self.needs_channel_asjustment:
            conv_fn = getattr(nn, f"Conv{self.dim}d")
            self.context_linear_layer = (
                Vmap(
                    conv_fn(
                        in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=4, stride=2, padding=1
                    )
                )
                if self.context_filled
                else None
            )
            self.target_linear_layer = conv_fn(
                in_channels=self.in_channels, out_channels=self.out_channels, kernel_size=4, stride=2, padding=1
            )
        else:
            pool_fn = getattr(nn, f"MaxPool{self.dim}d")
            self.context_pooling_layer = Vmap(pool_fn(kernel_size=2)) if self.context_filled else None
...
class UnetUpsampleAndConcatShortcutBlock(nn.Module):
    ...
    def __post_init__(self):
        ...
        conv_fn = getattr(nn, f"Conv{self.dim}d")
        self.context_conv_layer = (
            Vmap(
                conv_fn(
                    in_channels=self.in_channels + self.in_shortcut_channels,
        ...
        self.target_conv_layer = conv_fn(
            in_channels=self.in_channels + self.in_shortcut_channels,
            out_channels=self.out_channels,
            kernel_size=1,
            padding="same",
    ...
    def forward(
        self, context: torch.Tensor, target: torch.Tensor, shortcut: Tuple[torch.Tensor, torch.Tensor]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        # upsample
        context = vmap(self.upsampling_layer, context) if self.context_filled else context
        target = self.upsampling_layer(target)

        # concat with shortcut
        ctx_short, tgt_short = shortcut
        # B L C ...
        context = torch.cat([context, ctx_short], dim=2) if self.context_filled else context
        target = torch.cat([target, tgt_short], dim=1)  # B C ...

        # reduce dim
        context = self.context_conv_layer(context) if self.context_filled else context
...
if __name__ == "__main__":
    device = "cuda"
    unet2d = UnetBackbone(dim=2, stages=4, in_channels=2, out_channels=3, inner_channels=32).to(device)
    ...
    unet3d = UnetBackbone(dim=3, stages=4, in_channels=2, out_channels=3, inner_channels=32).to(device)
