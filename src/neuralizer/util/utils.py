...
def load_exampe(folder: str) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    for i in range(32):
        ...
        ctx_out.append(read_image(os.path.join(folder, f"ctx_out{i}.png")) / 255)
    ...
    print("CTX_in Shape in utils.load_exampe func: ", ctx_in.shape)
    if target.shape[0] == 1:
        target = pad_to_3_chan(target)
    if ctx_in.shape[1] == 1:
        ctx_in = pad_to_3_chan(ctx_in)
    if ctx_out.shape[1] == 1:
...
def corp_exampe(folder: str):
    ...
    torchvision.transforms.ToPILImage()(target).save(os.path.join(folder, "target_in.png"))
    for i in range(32):
        if not Path(folder, f"ctx_in{i}.png").is_file():
            break
        ...
        target = target[0, :192, :192]
        torchvision.transforms.ToPILImage()(target).save(os.path.join(folder, f"ctx_in{i}.png"))
        target = read_image(os.path.join(folder, f"ctx_out{i}.png"))
        target = target[0, :192, :192]
        torchvision.transforms.ToPILImage()(target).save(os.path.join(folder, f"ctx_out{i}.png"))
